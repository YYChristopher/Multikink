...
path1 <- "C:/Users/user/Documents/Graduation Project/code/Multikink R code and read data/SmoothKernel"
setwd(path1)
source("SmoothIter.R")
source("SmoothIter3.R")
source("SmoothVarEst.R")
...
Est1 <- SmoothMKQR2(x,y,z,tau1,h,bet.ini1, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
VarEst1 <- SmoothVar(y,x,z,tau1,h,Est1$coefficients,Est1$psi)
Est2 <- SmoothMKQR2(x,y,z,tau2,h,bet.ini2, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
VarEst2 <- SmoothVar(y,x,z,tau2,h,Est2$coefficients,Est2$psi)
Est3 <- SmoothMKQR2(x,y,z,tau3,h,bet.ini3, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
VarEst3 <- SmoothVar(y,x,z,tau3,h,Est3$coefficients,Est3$psi)
Est4 <- SmoothMKQR2(x,y,z,tau4,h,bet.ini4, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
VarEst4 <- SmoothVar(y,x,z,tau4,h,Est4$coefficients,Est4$psi)
Est5 <- SmoothMKQR2(x,y,z,tau5,h,bet.ini5, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
VarEst5 <- SmoothVar(y,x,z,tau5,h,Est5$coefficients,Est5$psi)
...
sink("TempSmooth.txt")
cat("Smooth,tau=0.1,Est1:","\n")
print(Est1)
print(VarEst1)
cat("Smooth,tau=0.3,Est2:","\n")
print(Est2)
print(VarEst2)
cat("Smooth,tau=0.5,Est3:","\n")
print(Est3)
print(VarEst3)
cat("Smooth,tau=0.7,Est4:","\n")
print(Est4)
print(VarEst4)
cat("Smooth,tau=0.9,Est5:","\n")
print(Est5)
print(VarEst5)
sink()