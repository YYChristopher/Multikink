...
path1 <- "C:/Users/user/Documents/Graduation Project/code/Multikink R code and read data/BS-QReg"
setwd(path1)
source("BSQreg2Iter.R")
source("BSQregVarEst.R")
...
Est1 <- BS.QReg2(x,y,z,tau1,h,bet.ini, bp.ini,alpha,tol=1e-8, max.iter=5000,yta,a,b)
VarEst1 <- BSQVar(y,x,z,tau1,h,Est1$coefficients,Est1$psi)
Est2 <- BS.QReg2(x,y,z,tau2,h,bet.ini, bp.ini,alpha,tol=1e-8, max.iter=5000,yta,a,b)
VarEst2 <- BSQVar(y,x,z,tau2,h,Est2$coefficients,Est2$psi)
Est3 <- BS.QReg2(x,y,z,tau3,h,bet.ini, bp.ini,alpha,tol=1e-8, max.iter=5000,yta,a,b)
VarEst3 <- BSQVar(y,x,z,tau3,h,Est3$coefficients,Est3$psi)
Est4 <- BS.QReg2(x,y,z,tau4,h,bet.ini, bp.ini,alpha,tol=1e-8, max.iter=5000,yta,a,b)
VarEst4 <- BSQVar(y,x,z,tau4,h,Est4$coefficients,Est4$psi)
Est5 <- BS.QReg2(x,y,z,tau5,h,bet.ini, bp.ini,alpha,tol=1e-8, max.iter=5000,yta,a,b)
VarEst5 <- BSQVar(y,x,z,tau5,h,Est5$coefficients,Est5$psi)
...
sink("TempBSQ.txt")
cat("BSQ,tau=0.1,Est1:","\n")
print(Est1)
print(VarEst1)
cat("BSQ,tau=0.3,Est2:","\n")
print(Est2)
print(VarEst2)
cat("BSQ,tau=0.5,Est3:","\n")
print(Est3)
print(VarEst3)
cat("BSQ,tau=0.7,Est4:","\n")
print(Est4)
print(VarEst4)
cat("BSQ,tau=0.9,Est5:","\n")
print(Est5)
print(VarEst5)
sink()