...
path1 <- "C:/Users/user/Documents/Graduation Project/code/Multikink R code and read data/InducedSmooth"
setwd(path1)
source("ISQregIter.R")
...
Est1 <- ISQReg(x,y,z,tau1,bet.ini, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
Est2 <- mkqr.bea(y, x, z, tau2,Cn=log(n))
Est2 <- ISQReg(x,y,z,tau2,bet.ini, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
Est3 <- ISQReg(x,y,z,tau3,bet.ini, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
Est4 <- ISQReg(x,y,z,tau4,bet.ini, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
Est5 <- ISQReg(x,y,z,tau5,bet.ini, bp.ini,tol=1e-8, max.iter=5000,yta,a,b)
...
PSI2 <- matrix(rep(Est2$psi.est,rep(n,k)),ncol=k)
xzu2 <- cbind(1,x,pmax(X-PSI2,0))
yt2 <- xzu2%*%Est2$bet.est
...
sink("TempInduced.txt")
cat("Induced,tau=0.1,Est1:","\n")
print(Est1)
cat("Induced,tau=0.3,Est2:","\n")
print(Est2)
cat("Induced,tau=0.5,Est3:","\n")
print(Est3)
cat("Induced,tau=0.7,Est4:","\n")
print(Est4)
cat("Induced,tau=0.9,Est5:","\n")
print(Est5)
sink()